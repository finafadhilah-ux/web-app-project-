...
import math 
# import pandas as pd # Biarkan ini dikomentari

# Fungsi Haversine Manual
...
## 1. DATA STRUCTURE (NODES and EDGES)
# --- DATA BARU DAN LENGKAP SESUAI PERMINTAAN ANDA ---

JAVA_NODES = {
    # West Java (Jawa Barat) - Kota & Kabupaten
    ...
    # Central Java (Jawa Tengah + DIY) - Kota & Kabupaten
    ...
    "Surakarta": {"lat": -7.5667, "lon": 110.8333, "province": "Central Java"}, # Ini Solo
    "KotaTegal": {"lat": -6.8778, "lon": 109.1126, "province": "Central Java"}, # Ini Tegal Kota
    "Yogyakarta": {"lat": -7.7956, "lon": 110.3695, "province": "Central Java"}, # Tambahkan DIY ke kluster Central Java
    
    # East Java (Jawa Timur) - Kota & Kabupaten
...
def create_network_graph(selected_province):
    ...
    for city, data in nodes_data.items(): 
    ...
    for u, v in edges_list:
        if G.has_node(u) and G.has_node(v):
            ...
            distance_km = haversine_distance(coord_u, coord_v)
            G.add_edge(u, v, weight=distance_km)
    return G
...
def render_map(G, path=None, center=(-7.0, 110.0), zoom_start=7):
    ...
    for u, v, d in G.edges(data=True):
        ...
        # Hanya tampilkan bobot jika path belum dipilih (untuk mengurangi clutter)
        tooltip_text = f"{u} - {v}"
        if path is None:
             tooltip_text += f" ({d['weight']:.2f} km)"
        folium.PolyLine(coords, weight=1.5, opacity=0.6, color='gray', tooltip=tooltip_text).add_to(m)
        
    # Nodes
    for n, d in G.nodes(data=True):
        folium.CircleMarker(location=[d["lat"], d["lon"]], radius=4, color='blue', fill=True, fill_color='blue', fill_opacity=0.8, popup=f"<b>{n}</b><br>{d['province']}").add_to(m)
    ...
    if path:
        ...
        # Marker Start
        folium.CircleMarker(location=[G.nodes[path[0]]["lat"], G.nodes[path[0]]["lon"]], radius=8, color='green', fill=True, fill_color='green', fill_opacity=1.0, popup=f"START: {path[0]}").add_to(m)
        # Marker End
        folium.CircleMarker(location=[G.nodes[path[-1]]["lat"], G.nodes[path[-1]]["lon"]], radius=8, color='darkred', fill=True, fill_color='darkred', fill_opacity=1.0, popup=f"END: {path[-1]}").add_to(m)
...
def main():
    ...
    # Set map center dan zoom berdasarkan provinsi yang dipilih
    ...
    if available_cities:
        st.header(f"Network Map: {selected_province}")
        m = render_map(G, center=map_center, zoom_start=map_zoom)
        folium_static(m)
        st.markdown("---"); st.header("Shortest Distance Calculator")
        
        col1, col2 = st.columns(2)
        with col1: start_city = st.selectbox("Start City", available_cities, index=available_cities.index("Bandung") if "Bandung" in available_cities else 0)
        with col2: end_city = st.selectbox("End City", [city for city in available_cities if city != start_city], index=available_cities.index("Surabaya") if "Surabaya" in available_cities else 1)
        ...
        if st.button("Calculate Shortest Path"):
            if start_city and end_city:
                try:
                    ...
                    st.success(f"✅ **Shortest Path Found!**"); st.info(f"**Total Distance:** {shortest_distance:.2f} KM")
                    st.write(f"**Path:** {' → '.join(shortest_path)}")
                    
                    st.subheader("Map with Shortest Path Highlighted")
                    m_path = render_map(G, path=shortest_path, center=map_center, zoom_start=map_zoom)
                    folium_static(m_path)
                    
                except nx.NetworkXNoPath: st.error(f"❌ No path found connecting {start_city} and {end_city}.")
                except Exception as e: st.error(f"An unexpected error occurred: {e}")
            else: st.warning("Please select both a Start City and an End City.")
    else: st.warning(f"No cities available to visualize in {selected_province}.")
